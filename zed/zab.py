...
class ZAB:
    ...
    def _initFromData(self, game, data):
        magic, fileLen, sectionCount, unk0C = struct.unpack_from('<4sIIi', data, 0)
        assert magic == b'ZCAB'
        ...
        # CABM
        cabmMagic, cabmSize, cabmUnk08, cabmUnk0C, cabmUnk0D, cabmCount = struct.unpack_from(
            '<4sIIBBH', data, offset
        )
    ...
    def save(self, game):
        ...
        # CABI
        data.extend(
            struct.pack('<4sIHh', b'CABI'[::-1], 12 + len(self.CABI) * 12, len(self.CABI), -1)
        )
        for c in self.CABI:
            data.extend(c)

        # ZAB header
        struct.pack_into('<4sIIi', data, 0, b'ZCAB', len(data), 2, -1)
