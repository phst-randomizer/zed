...
class CourseListInitMapEntry:
    ...
    def save(self, game):
        ...
        if game == common.Game.PhantomHourglass:
            return struct.pack('<4Bhh', self.mapID, 0xFF, 0xFF, 0xFF, unk01, unk02)
        else:
    ...
    def _initFromData(self, game, listData, initData):
        ...
        if game == common.Game.PhantomHourglass:
            (
                ...
                mapCount,
                self.phUnk38,
            ) = struct.unpack_from('<HHihh4Bi', listData, off)
            off += 0x14
            ...
            (
                ...
                self.unk26,
                self.unk27,
            ) = struct.unpack_from('<BbbbHbbH6b', listData, off)
            off += 0x10
        ...
        if game == common.Game.SpiritTracks:
            ...
            (
                ...
                self.initUnk24,
                self.initUnk26,
            ) = struct.unpack_from('<HHI6H', initData, 0x14)
            self.vehicleCourse = bool(vehicleCourse)
...
def loadCourseListAndInit(game, listData, initData=None):
    ...
    listMagic, listWrongFileLen, entriesCount1, entriesCount2 = struct.unpack_from(
        '<4s3I', listData, 0
    )
    if listMagic != b'ZCLB':
    ...
    if initData is not None:
        initMagic, initFileLen, entriesCount3, entriesCount4 = struct.unpack_from(
            '<4s3I', initData, 0
        )
        if initMagic != b'ZCIB':
    ...
    for i in range(entriesCount1):
        ...
        entries.append(CourseListInitCourseEntry(game, listEntryData, initEntryData))
