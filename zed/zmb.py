...
class Location:
    ...
    def __init__(self, game=None, data=None):
        ...
        if data is None:
            ...
            (self.id, unk01, unk02, self.y1, self.x1, self.y2, self.x2) = struct.unpack_from(
                '<BB5h', data
            )
...
class Entrance:
    ...
    def __init__(self, game=None, data=None):
        if data is None:
            ...
            (self.x, self.z, self.y, self.rotation, self.id, self.unk0F) = struct.unpack_from(
                '<IIIhbb', data
            )
    ...
    def save(self, game):
        ...
        if game == common.Game.SpiritTracks:
            ...
            return struct.pack(
                '<IIIhbb', self.x, self.z, self.y, self.rotation, self.id, self.unk0F
            )
...
class ZMB:
    ...
    def _initFromData(self, game, data):
        ...
        magic1, magic2, fileLen, sectionCount, unk10, unk14, unk18, unk1C = struct.unpack_from(
            '<4s4s6I', data
        )
    ...
    def renderPNG(self):
        ...
        for directory in font_directories:
            try:
                font = PIL.ImageFont.truetype(
                    str(pathlib.Path(directory) / 'NotoMono-Regular.ttf'), size=10
                )
                break
            except OSError:
                font = None
        ...
        if stuff:
            ...
            def canvasPos(x, y) -> tuple[float, float]:
                return (1024 * (x - canvasX) / canvasW, 1024 * (y - canvasY) / canvasH)
...
def drawRect(img, xy, fill=None, outline=None):
    ...
    rdraw.rectangle([(0, 0), (w, h)], fill=fill, outline=outline)
